...
**Design:** record a per-(class,key) "resolved-but-declined-external" marker in a **boot-scope-lived set** (mirror the `SeedResolveMemo` context-carried idiom — installed by the boot scope, torn down when the boot scope's context ends, never on the /call path). In `seedSkipDecision(seedModeBoot, ...)`, before the `handle.Get`, consult the marker: if this key was already resolved-and-declined *this boot*, skip (the cell is intentionally cold; re-resolving it every resume pass makes zero forward progress and only burns budget). Set the marker in `declineSeedPutOnError`'s external-touch branch.

- **Placement:** the marker set is installed alongside the memo in `withCohortSeedContext` (`phase1_pip_seed.go:433`, boot-scope only); consulted in `seedSkipDecision` case `seedModeBoot` (`:529`); populated in `declineSeedPutOnError` external branch (`:1178`). Key = the same `key` the Put/Get use (single-derivation, no drift — same discipline as `feedback_consultation_mutation_is_not_key_correctness`).
- **Why this is safe / correct:** it only skips on *resume passes within the same boot scope* (the set is fresh per boot-scope context). The FIRST boot pass still resolves each external whale once (records the served content for first-paint via the /call cold path lazily — the cell was already going to be cold-on-first-/call anyway, since the Put was declined). It does not warm an un-invalidatable cell (the whole point of the #102 decline is preserved). It changes only whether the seed *wastes budget re-resolving a cell it will decline again*.
